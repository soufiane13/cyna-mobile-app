...
export default function HomeScreen() {
    ...
    const loadData = async () => {
      try {
        ...
        const [productRes, alertRes] = await Promise.all([
          api.get('/products').catch(() => ({ data: [] })), // Adapte la route si besoin
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      {alertMessage ? (
        <View style={styles.alertBanner}>
          <View style={styles.alertDotContainer}>
            <View style={styles.alertDotPing} />
      ...
      {/* ── HERO SECTION (Simplifiée pour mobile) ── */}
...
const styles = StyleSheet.create({
  ...
  alertDotPing: { position: 'absolute', width: 12, height: 12, borderRadius: 6, backgroundColor: '#06b6d4', opacity: 0.5 }, // Animation "ping" non native, gérée par l'opacité fixe ici
  ...
  heroTitle: { color: '#fff', fontSize: 24, fontWeight: 'black', textAlign: 'center', marginBottom: 10 },
  ...
  loadingContainer: { py: 40, alignItems: 'center' },
